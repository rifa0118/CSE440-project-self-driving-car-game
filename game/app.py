...
class SelfDrivingCarApp:
    ...
    def __init__(self) -> None:  # pragma: no cover - requires Pygame
        ...
        self.obstacle_count_index = OBSTACLE_COUNT_CHOICES.index(3)
        self.speed_index = 2
    ...
    @property
    def selected_obstacle_count(self) -> int:
        return OBSTACLE_COUNT_CHOICES[self.obstacle_count_index]
    ...
    SETTINGS_BACK_INDEX = 4

    def _settings(self, events: list[Any]) -> None:
        for event in events:
            if event.type == self.pg.KEYDOWN:
                if event.key == self.pg.K_UP:
                    self.settings_index = (self.settings_index - 1) % self.SETTINGS_OPTION_COUNT
                    ...
                    if self.settings_index == self.SETTINGS_BACK_INDEX:
                        self.mode = "menu"
                    else:
                ...
                if index is not None:
                    ...
                    if index == self.SETTINGS_BACK_INDEX:
                        self.mode = "menu"
                    else:
                        self._change_setting(1)
        ...
        if self.mode == "settings":
            self._click_rects = self.renderer.draw_settings(
                ...
                obstacle_count=self.selected_obstacle_count,
    ...
    def _change_setting(self, direction: int) -> None:
        if self.settings_index == 0:
            ...
            self.obstacle_count_index = (
                self.obstacle_count_index + direction
            ) % len(OBSTACLE_COUNT_CHOICES)
    ...
    def _manual(self, events: list[Any]) -> None:
        ...
        self.renderer.draw_game(
            ...
            stats={
                ...
                "Obstacles": getattr(self.env.config, "obstacle_count", 0),
