...
def cast_sensors(
    ...
    traffic: list[Car] | None = None,
) -> SensorReading:
    """Cast all configured rays until each one reaches a wall, traffic, or max range."""
    ...
    for sensor_index, relative_angle in enumerate(config.sensor_angles_deg):
        ...
        traffic_hit_idx = len(distances_to_test)
        if traffic:
            for i, pt in enumerate(points):
                hit = False
                for t_car in traffic:
                    if np.linalg.norm(pt - t_car.position) < 23.0:
                        traffic_hit_idx = i
                        hit = True
                        break
                if hit:
        ...
        first_hit = min(wall_hit_idx, traffic_hit_idx)
