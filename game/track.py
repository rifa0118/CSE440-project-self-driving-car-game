...
def _make_centerline(name: str, width: int, height: int) -> tuple[np.ndarray, int]:
    ...
    if name == "easy":
        ...
        x = cx + 480.0 * radial * np.cos(t)
        y = cy + 280.0 * (1.0 + 0.11 * np.cos(2.0 * t - 0.4)) * np.sin(t)
        road_width = 98
    elif name == "hard":
        radial_x = 1.0 + 0.14 * np.sin(3.0 * t + 0.45) + 0.055 * np.sin(5.0 * t)
        radial_y = 1.0 + 0.13 * np.cos(4.0 * t - 0.25) - 0.035 * np.sin(2.0 * t)
        x = cx + 460.0 * radial_x * np.cos(t)
        y = cy + 280.0 * radial_y * np.sin(t)
        road_width = 84
...
def _draw_start_line(
    ...
    stripe_depth = 12.0
    # Make a 3x10 checkerboard
    for row in range(3):
        for column in range(10):
            colour = (240, 240, 240) if (row + column) % 2 == 0 else (20, 20, 20)
            across_0 = -half_width + column * tile_width
            across_1 = across_0 + tile_width
            along_0 = -stripe_depth * 1.5 + row * stripe_depth
...
def _background(width: int, height: int, seed: int) -> Image.Image:
    ...
    base = Image.new("RGB", (width, height), (50, 140, 70))
    ...
    for x in range(0, width, grid_size):
        for y in range(0, height, grid_size):
            if (x // grid_size + y // grid_size) % 2 == 0:
                draw.rectangle((x, y, x + grid_size, y + grid_size), fill=(45, 130, 60))

    for _ in range(300):
        x = int(rng.integers(0, width))
        y = int(rng.integers(0, height))
        radius = int(rng.integers(1, 4))
        colour = (35, 110, 50) if rng.random() > 0.5 else (60, 160, 80)
