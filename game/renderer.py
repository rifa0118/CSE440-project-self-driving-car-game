...
class PygameRenderer:
    ...
    def _draw_obstacle(self, obstacle) -> None:  # pragma: no cover
        """Draw one static obstacle, styled according to its kind."""
        drawers = {
            "cone": self._draw_obstacle_cone,
            "barrel": self._draw_obstacle_barrel,
            "barrier": self._draw_obstacle_barrier,
            "rock": self._draw_obstacle_rock,
            "crate": self._draw_obstacle_crate,
        }
        drawer = drawers.get(obstacle.kind, self._draw_obstacle_cone)
        drawer(obstacle)

    def _obstacle_axes(self, obstacle) -> tuple[np.ndarray, np.ndarray]:
        heading = np.array(
            [np.cos(np.radians(obstacle.angle_deg)), np.sin(np.radians(obstacle.angle_deg))],
            dtype=np.float32,
        )
        normal = np.array([-heading[1], heading[0]], dtype=np.float32)
        return heading, normal

    def _draw_obstacle_shadow(self, screen_points: list[tuple[int, int]]) -> None:  # pragma: no cover
        pg = require_pygame()
        shadow = [(x + 3, y + 4) for x, y in screen_points]
        pg.draw.polygon(self.screen, (16, 18, 22), shadow)

    def _draw_obstacle_cone(self, obstacle) -> None:  # pragma: no cover
        """A road cone obstacle: a small orange triangle with a white stripe."""
        pg = require_pygame()
        heading, normal = self._obstacle_axes(obstacle)
        position = obstacle.position
        radius = obstacle.radius

        def to_screen(rel_x, rel_y):
            pt = position + heading * (radius * rel_x) + normal * (radius * rel_y)
            return self._world_to_screen(pt)

        base = [to_screen(rx, ry) for rx, ry in ((0.6, 0.0), (-0.5, 0.7), (-0.5, -0.7))]
        self._draw_obstacle_shadow(base)
        pg.draw.polygon(self.screen, (235, 120, 30), base)
        pg.draw.polygon(self.screen, (30, 20, 10), base, width=1)
        stripe = [to_screen(rx, ry) for rx, ry in ((-0.05, 0.32), (-0.25, 0.42), (-0.25, -0.42), (-0.05, -0.32))]
        pg.draw.polygon(self.screen, (240, 240, 240), stripe)

    def _draw_obstacle_barrel(self, obstacle) -> None:  # pragma: no cover
        """A parked construction barrel: an orange/white striped cylinder seen from above."""
        pg = require_pygame()
        position = obstacle.position
        radius = obstacle.radius
        centre = self._world_to_screen(position)
        shadow_centre = (centre[0] + 2, centre[1] + 3)
        pg.draw.circle(self.screen, (16, 18, 22), shadow_centre, int(radius * 0.8))
        pg.draw.circle(self.screen, (210, 80, 20), centre, int(radius * 0.8))
        pg.draw.circle(self.screen, (240, 240, 240), centre, int(radius * 0.55), width=3)
        pg.draw.circle(self.screen, (40, 25, 15), centre, int(radius * 0.8), width=2)

    def _draw_obstacle_barrier(self, obstacle) -> None:  # pragma: no cover
        """A long red-and-white road barrier block."""
        pg = require_pygame()
        heading, normal = self._obstacle_axes(obstacle)
        position = obstacle.position
        radius = obstacle.radius
        ...
        body = [to_screen(rx, ry) for rx, ry in ((0.55, 0.35), (0.55, -0.35), (-0.55, -0.35), (-0.55, 0.35))]
        self._draw_obstacle_shadow(body)
        pg.draw.polygon(self.screen, (225, 225, 225), body)
        pg.draw.polygon(self.screen, (60, 60, 60), body, width=1)
        for stripe_x in (-0.3, 0.0, 0.3):
            stripe = [
                to_screen(stripe_x - 0.08, 0.35),
                to_screen(stripe_x + 0.08, 0.35),
                to_screen(stripe_x + 0.08, -0.35),
                to_screen(stripe_x - 0.08, -0.35),
            ]
            pg.draw.polygon(self.screen, (210, 40, 40), stripe)

    def _draw_obstacle_rock(self, obstacle) -> None:  # pragma: no cover
        """An irregular grey boulder."""
        pg = require_pygame()
        heading, normal = self._obstacle_axes(obstacle)
        position = obstacle.position
        radius = obstacle.radius
        ...
        outline = [
            (0.55, 0.10), (0.30, 0.55), (-0.20, 0.60), (-0.55, 0.15),
            (-0.45, -0.40), (0.05, -0.60), (0.50, -0.30),
        ]
        points = [to_screen(rx, ry) for rx, ry in outline]
        self._draw_obstacle_shadow(points)
        pg.draw.polygon(self.screen, (120, 118, 116), points)
        pg.draw.polygon(self.screen, (78, 76, 74), points, width=2)
        crack = [to_screen(rx, ry) for rx, ry in ((0.15, 0.20), (-0.10, -0.10), (0.10, -0.35))]
        pg.draw.lines(self.screen, (78, 76, 74), False, crack, 2)

    def _draw_obstacle_crate(self, obstacle) -> None:  # pragma: no cover
        """A wooden supply crate, seen from above."""
        pg = require_pygame()
        heading, normal = self._obstacle_axes(obstacle)
        position = obstacle.position
        radius = obstacle.radius
        ...
        body = [to_screen(rx, ry) for rx, ry in ((0.45, 0.45), (0.45, -0.45), (-0.45, -0.45), (-0.45, 0.45))]
        self._draw_obstacle_shadow(body)
        pg.draw.polygon(self.screen, (150, 105, 60), body)
        pg.draw.polygon(self.screen, (85, 58, 30), body, width=2)
        diagonal_a = [to_screen(rx, ry) for rx, ry in ((-0.45, -0.45), (0.45, 0.45))]
        diagonal_b = [to_screen(rx, ry) for rx, ry in ((-0.45, 0.45), (0.45, -0.45))]
        pg.draw.line(self.screen, (85, 58, 30), diagonal_a[0], diagonal_a[1], 2)
        pg.draw.line(self.screen, (85, 58, 30), diagonal_b[0], diagonal_b[1], 2)
